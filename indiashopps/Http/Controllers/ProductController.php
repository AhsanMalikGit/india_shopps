...
class ProductController extends Controller 
{
	...
	public function category( $cat_name )
	{
		...
		$cat_data 		= DB::table('gc_cat')->where(DB::raw(" create_slug(name) "),"like", $cat_name)->where('level',0)->select('id','seo_title','meta')->first();
	...
	//print_r($data );exit;
		...
		$data['slider'] = DB::table('slider')->where('refer_id',$cat_data->id)->select('image_url','refer_url','alt')->get();
	...
	{
		$data['categories'] = Helper::get_categories_tierd(); // \App\Helpers\helper.php
	...
	public function trending()
	{
		...
		// Gets Data from SOLR... @composer_url ==> Check image.php file.. \App\Helpers\image.php
...
//echo "<pre>";print_r($data);exit;
	...
	*/
	...
	function product_sitemap()
	{
		//$cats	= $this->get_categories_tierd();
		// dd($cats);
		$priority = 0.8; $url = '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">';		
		$from = 0;
		do
		{
		...
		$url .= "</urlset>";
		file_put_contents("product-sitemap.xml", $url );
	}
	
	function image_sitemap()
	{
		$priority = 0.6; $url = '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">';		
		$from = 20500;
	...
	function get_categories($parent = false)
	{
		...
		if ($parent !== false)
		{
			...
			$result	= DB::table('gc_cat')->where('parent_id',$parent)->where('active',1)->where('level','<',3)->orderBy('sequence', 'ASC')->lists('id');
		}
		else
		{
	 ...
	 * @var Product Name
	 */
	public function list_products(Request $request,$brand=false,$cat=false,$order_by="null",$sort_order="null",$page=0)
	{
