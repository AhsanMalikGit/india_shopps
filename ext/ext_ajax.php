...
if(isset($_POST['cat']) && !empty($_POST['cat']) && (trim($_POST['cat']) == 'Smartphones' || trim($_POST['cat']) == 'Basic Mobiles'|| trim($_POST['cat']) == 'Mobile Phones'))
{
	$cat = "+(category:Mobiles)";
}
if(!empty($prod))
{
	//if(strpos($prod,'MOB') !== false)
	if(substr( $prod, 0, 3 ) === 'MOB')
	{
		$cat = "+(category:Mobiles)";
		//$cat = "+category:Mobiles";
	}
	$cat = $cat."-(product_id:$prod)";
}
...
//$searchAPI = 'http://103.240.167.5:9200/shopping/_search';
$searchAPI = 'http://103.252.96.182:9200/shopping/_search';
//echo $searchAPI.'?q='.urlencode($productInfo).$cat."&size=4";exit;
if(!empty($cat))
{
	$serializedResult = file_get_contents($searchAPI.'?q='.urlencode($productInfo).$cat."&size=4");
}else{
