...
if(isset($cat_post) && !empty($cat_post) && (trim($cat_post) == 'Smartphones' || trim($cat_post) == 'Basic Mobiles'|| trim($cat_post) == 'Mobile Phones' || trim($cat_post) == 'Mobiles'))
{
	$cat = "Mobiles";	
	...
	//if($vendor != 1 && $vendor !=2 && $vendor !=3  && $vendor !=5 )
	{
		...
		foreach($result['hits']['hits'] as $val){ 
			if($i <= 8)
			{
				...
				if($val['vendor']==16 || $val['vendor']==23)
				{
